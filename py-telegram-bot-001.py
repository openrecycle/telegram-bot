    ...
    if district != None:
        ...
        with codecs.open('districts.csv', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                if result != "":
                    result += "\n"
                if district.lower() == row[0].lower().encode('utf-8') or district.lower() == row[0].lower()[
                                                                                             :len(district)]:
                    result += row[1]
    return result
    return result
    ...
    if metro_name != None:
        ...
        with codecs.open('districts.csv', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                if result != "":
                    result += "\n"
                if metro_name.lower() == row[1].lower().encode('utf-8') or metro_name.lower() == row[1].lower()[:len(
                        metro_name)] or metro_name.lower() in row[1].lower():
                    result += row[1]
    return result
